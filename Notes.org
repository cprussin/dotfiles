...
** TODO Stop using emacs-overlay and remove emacs from unstable-pkgs (when emacs 29 with pgtk lands in nixpkgs)
** TODO Store luks keys somewhere that isn't visible to non-root accounts
** TODO Install exa (ls replacement) and bat (cat replacement) and nushell (zsh replacement)
** TODO Manage gemini luks keys with colmena
** TODO Don't store crux luks keys on /boot at all (send via ssh on boot)
** TODO Make install-user-mountpoints not suck
** TODO Only open crux firewall ports to wireguard interface
